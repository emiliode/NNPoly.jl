...
function dense_to_monomon(bern_dense, bern_implicit::NP.BernsteinPolynomialImp)
    ...
    res = 0
    basis_vectors = []
    push!(basis_vectors, [subs(p, variables(p)[1] => x) for p in bern_implicit.bernstein_basis[1]] )
    push!(basis_vectors, [subs(p, variables(p)[1] => y) for p in bern_implicit.bernstein_basis[2]] )
    for I in CartesianIndices(bern_dense)
        print("I=$I")
        prod = bern_dense[I]
        for i in 1:bern_implicit.n
...
function imp_to_monomon(bern_imp::NP.BernsteinPolynomialImp,x ) 
    ...
    for i in 1:length(x)
        push!(bern_eval,  [[ p(variables(p)[1] => x[i])  for p in  bernstein_basis[i] ]; zeros(maximum(bern_imp.orders)-bern_imp.orders[i]) ] )
    end
