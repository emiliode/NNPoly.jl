...
acas_path = joinpath(
    @__DIR__,
    "../../vnncomp2022_benchmarks/benchmarks/acasxu/onnx/ACASXU_run2a_1_1_batch_2000.onnx",
);
#vnnlib_path = "../../vnncomp22/acasxu/vnnlib/prop_1.vnnlib"
vnnlib_path = joinpath(
    @__DIR__,
    "../../vnncomp2022_benchmarks/benchmarks/acasxu/vnnlib/prop_1.vnnlib",
)
...
t = @elapsed res, lbs_pcrown, ubs_pcrown = NP.optimise_bounds(
    ...
    net_pcrown,
    input_set_acas,
    params = NP.OptimisationParams(print_freq = 25, timeout = 300, n_steps = 5000),
)
...
@variable(
    model_input,
    low(input_set_acas)[i] <= x_in[i=1:dim(input_set_acas)] <= high(input_set_acas)[i]
)
...
model_inset =
    NP.encode_network(enc, net_pruned, model_input, lbs_pcrown, ubs_pcrown, verbosity = 1)
