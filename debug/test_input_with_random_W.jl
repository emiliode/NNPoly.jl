...
for _ = 1:100
    ...
    for ((bern_low, bern_up), (poly_low, poly_up)) in zip(
        zip(
            multi_to_list(res.Low, res.n, res.X, x),
            multi_to_list(res.Up, res.n, res.X, x),
        ),
        zip(mapped_poly_low, mapped_poly_up),
    )
        println("testing lower")
        println("poly_low: $poly_low")
        println("bern_low: $bern_low")
        test_poly_equal(bern_low, poly_low)
        println("testing upper")
        test_poly_equal(bern_up, poly_up)
    end
end
