...
function bernstein_basis(order::Int64, a::T,b::T)::Vector{AbstractPolynomialLike} where {T<:Number}
    basis = []
    @polyvar x
    for k in 0:order 
...
"""
function get_basis_vectors(orders::Vector{Int64}, X::Hyperrectangle{T})::Vector{Vector{AbstractPolynomialLike}} where {T<:Number} 
    return [bernstein_basis(order,center(X)[i] - radius_hyperrectangle(X,i), center(X)[i]+radius_hyperrectangle(X,i)) for (i,order) in enumerate(orders)] 
end
"""
...
function make_polynomial(polynom::AbstractPolynomialLike, orders::Vector{Int64}, X::Hyperrectangle{T}) where {T<:Number}
    ...
    return BernsteinPolynomialImp(bern_coeff_matrix,length(orders),length(terms(polynom)),X,orders)
end
    
function evaluate(bern_imp::BernsteinPolynomialImp, point::AR) where {N<:Number, AR<:AbstractArray{N}} 
    i = 0
    bern_eval = []
    for row in bern_imp.bernstein_basis 
        push!(bern_eval,  [[ p(variables(p)[1] => point[i+1])  for p in row ]; zeros(maximum(bern_imp.orders)-bern_imp.orders[i+1]) ] )
        i = (i +1 ) % bern_imp.n
    end
    print("bern_eval: $bern_eval");

    eval = 0
    for term_idx in 0:(bern_imp.t -1)
        term_eval = 1
        for var_idx in 1:bern_imp.n 
...
function scalar_mul(bern_poly::BernsteinPolynomialImp, scalar::Number)  
    ...
    for term_idx in 0:(bern_poly.t - 1)
        new_coeff_mat[(term_idx*bern_poly.n)+1,:] = bern_poly.coefficient_matrix[(term_idx*bern_poly.n)+1,:] .* scalar 
    end 
    return BernsteinPolynomialImp(new_coeff_mat,bern_poly.n,bern_poly.t,bern_poly.X,bern_poly.orders)
end
...
function translate(poly::BernsteinPolynomialImp,b::Number)
    ...
    b_coeff_mat[1,:] .*= b
    return BernsteinPolynomialImp(vcat(poly.coefficient_matrix,b_coeff_mat),poly.n,poly.t+1,poly.X,poly.orders)
end
...
function elevate_degree(bern_imp::BernsteinPolynomialImp, new_orders::Vector{Int64})::BernsteinPolynomialImp
...
function row_convolution_kernel(T1::AbstractMatrix, T2::AbstractMatrix)
    ...
    for row in 1:T1_rows
        for col in 1:result_cols
...
multiply two terms
"""
function multiply_term(term_a::Matrix,orders_a::Vector{Int64}, term_b::Matrix, orders_b::Vector{Int64})::Matrix 
...
"""
function multiply(bern_a::BernsteinPolynomialImp, bern_b::BernsteinPolynomialImp)::BernsteinPolynomialImp
    res = []
    for i in 0:(bern_a.t-1)
        term_a = bern_a.coefficient_matrix[(i*bern_a.n + 1):(i*bern_a.n)+bern_a.n,:] 
        for j in 0:(bern_b.t-1)
    ...
    #println(res)
    return BernsteinPolynomialImp(reduce(vcat,res),bern_a.n,bern_a.t*bern_b.t,bern_a.X, bern_a.orders+bern_b.orders)
end
...
"""
function add(bern_a::BernsteinPolynomialImp, bern_b::BernsteinPolynomialImp)::BernsteinPolynomialImp
    ...
    return BernsteinPolynomialImp(vcat(elevated_bern_a.coefficient_matrix , elevated_bern_b.coefficient_matrix),elevated_bern_a.n,elevated_bern_a.t+elevated_bern_b.t,elevated_bern_a.X,res_orders)
end
