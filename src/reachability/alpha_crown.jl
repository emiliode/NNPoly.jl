...
function backward_linear(
    solver::aCROWN,
    L::Union{NV.Layer,NV.LayerNegPosIdx,CROWNLayer},
    input::SymbolicBound,
)
    Λ = input.Λ * L.weights
...
function initialize_slopes!(
    solver::aCROWN,
    lbs::VN,
    ubs::VN,
    α::VN,
) where {N<:Number,VN<:AbstractVector{N}}
...
"""
function backward_act(
    solver::aCROWN,
    L::CROWNLayer{NV.ReLU,MN,BN,AN},
    input::SymbolicBound{N},
    lbs,
    ubs;
    upper = false,
) where {MN,BN,AN,N<:Number}
...
function backward_act(
    solver::aCROWN,
    L::Union{NV.Layer{NV.Id},NV.LayerNegPosIdx{NV.Id},CROWNLayer{NV.Id,MN,BN,AN}},
    input::SymbolicBound,
    lbs,
    ubs;
    upper = false,
...
function NV.forward_network(
    ...
    for (i::Int, l) in enumerate(net.layers[from_layer:end])
        ...
        ll, lu = bounds(Zl.Λ, Zl.γ, input_set; method=bounds_method, threshold)
        ul, uu = bounds(Zu.Λ, Zu.γ, input_set; method=bounds_method, threshold)
...
function initialize_params_bounds(solver::aCROWN, net, degree::N, input; bounds_method=Overapproximate, threshold=-1) where {N<:Number}
    ...
    ŝ = NV.forward_network(isolver, net, input, lbs, ubs;bounds_method, threshold)
...
function optimise_bounds(
    ...
    opt = Optimisers.OptimiserChain(Optimisers.Adam(), Projection(0.0, 1.0)),
    params::OptimisationParams = OptimisationParams(),
    loss_fun = bounds_loss,
    print_result = false,
)
