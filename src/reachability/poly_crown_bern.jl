...
@with_kw struct PolyCROWNBern <: NV.Solver
    ...
    prune_neurons = false  # prune fixed inactive ReLUs
    ...
    interval_repr  = Dense
    bounds_method  = Overapproximate # only relevant if interval_repr is not dense 
    threshold = 50; # only used for SmithBounds 
end


function PolyCROWNBern(
    psolver;
    separate_alpha = true,
    use_tightened_bounds = true,
    initialize = false,
...
function NV.forward_network(
    ...
    net_poly::NN,
    net::NN,
    input_set::BernsteinInterval{N,M,O,TN};
    from_layer = 1,
    lbs,
    ubs,
    printing = false,
) where {NN<:Union{NV.Network,NV.NetworkNegPosIdx}, N<:Number, M<:Integer,O<:Number, TN<:AbstractArray{O}}
    ...
    αs_polyₗ = [zeros(3) for _ in 1:length(net.layers)]
    αs_polyᵤ = [zeros(3) for _ in 1:length(net.layers)]
    ...
    for (i, l) in enumerate(net.layers[from_layer:end])
        ...
        if solver.poly_solver.common_generators
            ...
            l_poly = add(
                translate(linear_map(max.(0, Zl.Λ), Lₗ), Zl.γ),
                linear_map(min.(0, Zl.Λ), Uₗ),
            )
            u_poly = add(
                translate(linear_map(max.(0, Zu.Λ), Uᵤ), Zu.γ),
    ...
    return BernsteinInterval(l_poly, u_poly, input_set.n, input_set.X)
end
...
function initialize_symbolic_domain(
    ...
    return initialize_symbolic_domain(solver.poly_solver, net[1:solver.poly_layers], input; repr)
end
...
function initialize_params_bounds(
    ...
    ipsolver = BernSym(
        ...
        bounds_method = solver.bounds_method,
    ...
    lbs_lin, ubs_lin = initialize_params_bounds(solver.lin_solver, net[(solver.poly_layers+1):end], 1, s_poly; bounds_method=solver.bounds_method, threshold=solver.threshold)
    return ŝ,
    [poly_lbs; lbs_lin],
    [poly_ubs; ubs_lin]
    #rs,
...
function propagate(
    ...
    ŝ, lbs, ubs = initialize_params_bounds(solver,net,2,input)

    s_poly = forward_act_stub(solver.poly_solver, net[1], ŝ, lbs[1], ubs[1])
